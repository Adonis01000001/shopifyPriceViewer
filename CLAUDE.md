...
1. Copy `.env.example` to `.env` and fill in `DATABASE_URL`, `JWT_SECRET` (>=32 chars in prod), `ENCRYPTION_KEY_SALT`, `SHOPIFY_API_KEY`, `SHOPIFY_API_SECRET`. Generate secrets with `node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"`.
...
JWT production validation, PBKDF2 key derivation, encrypted tokens/passwords at rest, no-shopify-token-to-frontend (the old `getToken` endpoint was removed — see comment in `routers.ts`), defense-in-depth rate limiting (general + auth + OAuth-specific + per-route scrape limiter), Helmet headers, CSRF only on Express OAuth routes (tRPC relies on JWT httpOnly cookies), 24h JWT + refresh tokens, `secure` cookie behind `connection.encrypted`, storage proxy path validation, CORS prod origin allowlist.
