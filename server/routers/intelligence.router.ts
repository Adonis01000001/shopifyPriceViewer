...
export const intelligenceRouter = router({
  ...
  discoverCompetitors: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      return competitorDiscoveryService.discoverForProduct(
        ctx.user!.id,
        input.productId,
        {
  ...
  discoverAllProducts: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const results = await competitorDiscoveryService.discoverForAllProducts(
        ctx.user!.id,
        {
          country: input?.country,
          language: input?.language,
    ...
    .input(
      z
        .object({
          ...
          status: z
            .enum(["pending", "verified", "rejected", "imported"])
            .optional(),
  ...
  approveDiscovery: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const result = await competitorDiscoveryService.approveDiscovery(
        ctx.user!.id,
        input.discoveryId
      );
      if (!result)
  ...
  rejectDiscovery: protectedProcedure
    .input(z.object({ discoveryId: z.string().uuid() }))
    .mutation(async ({ ctx, input }) => {
      const result = await competitorDiscoveryService.rejectDiscovery(
        ctx.user!.id,
        input.discoveryId
      );
      if (!result)
  ...
  importDiscoveryAsCompetitor: protectedProcedure
    .input(
      z.object({ discoveryId: z.string().uuid(), productId: z.string().uuid() })
    )
    .mutation(async ({ ctx, input }) => {
      const discoveries = await competitorDiscoveryService.getDiscoveries(
        ctx.user!.id,
      ...
      await competitorDiscoveryService.approveDiscovery(
        ctx.user!.id,
        input.discoveryId
      );
      return competitor;
