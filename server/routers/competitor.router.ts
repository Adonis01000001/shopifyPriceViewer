...
import { competitorProducts, products, priceHistory, competitors, activityLogs } from "../../drizzle/schema";
...
export const competitorRouter = router({
  ...
  getById: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      const competitor = await competitorService.getById(ctx.user!.id, input.id);
      if (!competitor) throw new TRPCError({ code: "NOT_FOUND", message: "Competitor not found" });
      return competitor;
    }),
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!competitor) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Failed to create competitor" });
      return competitor;
    }),
  ...
  feed: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      const feed = await competitorService.getFeed(ctx.user!.id, input.competitorId, input.limit ?? 50);
      if (!feed) throw new TRPCError({ code: "NOT_FOUND", message: "Competitor not found" });
      return feed;
    }),
  ...
  addProduct: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!product) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Failed to add competitor product" });
      return product;
    }),
  ...
  scrapeProducts: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const comp = await competitorService.getById(ctx.user!.id, input.competitorId);
  ...
  removeProduct: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const removed = await competitorService.removeProduct(ctx.user!.id, input.competitorProductId);
      if (!removed) throw new TRPCError({ code: "NOT_FOUND", message: "Product link not found" });
      return { success: true };
    }),
