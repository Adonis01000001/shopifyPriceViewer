...
export const notificationPreferences = pgTable(
  "notification_preferences",
  {
    ...
    priceDropThreshold: decimal("price_drop_threshold", { precision: 5, scale: 2 }).default("5.00"),
    priceIncreaseThreshold: decimal("price_increase_threshold", { precision: 5, scale: 2 }).default("5.00"),
    createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
    updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
  },
  (t) => ({
...
export const shopifyStores = pgTable(
  ...
  (t) => ({
    ...
    shopDomainIdx: uniqueIndex("shopify_stores_shop_domain_idx").on(t.shopDomain),
  })
);
...
export const products = pgTable(
  ...
  (t) => ({
    ...
    shopifyProductIdIdx: index("products_shopify_product_id_idx").on(t.shopifyProductId),
  })
);
...
export const productEmbeddings = pgTable(
  ...
  (t) => ({
    productIdIdx: uniqueIndex("product_embeddings_product_id_idx").on(t.productId),
  })
);
...
export const competitors = pgTable(
  "competitors",
  {
    ...
    avgPriceDiff: decimal("avg_price_diff", { precision: 5, scale: 2 }).default("0.00"),
    priceIndex: decimal("price_index", { precision: 5, scale: 2 }).default("100.00"),
...
export const competitorProducts = pgTable(
  "competitor_products",
  {
    ...
    competitorProductTitle: varchar("competitor_product_title", { length: 500 }),
  ...
  (t) => ({
    competitorIdIdx: index("competitor_products_competitor_id_idx").on(t.competitorId),
    productIdIdx: index("competitor_products_product_id_idx").on(t.productId),
    matchScoreIdx: index("competitor_products_match_score_idx").on(t.matchScore),
    uniqueMatchIdx: uniqueIndex("competitor_products_unique_idx").on(t.competitorId, t.productId),
  })
);
...
export const priceHistory = pgTable(
  ...
  (t) => ({
    ...
    competitorProductIdIdx: index("price_history_competitor_product_id_idx").on(t.competitorProductId),
    recordedAtIdx: index("price_history_recorded_at_idx").on(t.recordedAt),
    productRecordedIdx: index("price_history_product_recorded_idx").on(t.productId, t.recordedAt),
  })
);
...
export const recommendations = pgTable(
  ...
  (t) => ({
    ...
    confidenceIdx: index("recommendations_confidence_idx").on(t.confidenceScore),
